...
(defn- is-valid?
  ...
  (every?
    (fn [clue] ;; Função anônima que roda para CADA regra.
      ...
      ;; Pega os dados para a verificação de dentro da regra atual (que é um mapa).
      (let [sum-target (:sum clue)            ;; O alvo da soma (ex: 16)
            cells (:cells clue)            ;; A lista de células (ex: '([1 2] [1 3])) (coordenadas)
            
            ;; Pega os valores atuais do tabuleiro para essas células (valores já preenchidos). 
