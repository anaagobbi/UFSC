...
void mult_matrix(double* out, double* left, double *right, int rows_left, int cols_left, int cols_right) {
    // não faz muito sentido declarar as variáveis fora do loop e depois torná-las privadas
    // int i, j, k
    // #pragma omp parallel for schedule(dynamic, 1)
    #pragma omp parallel for schedule(static)
    for (int i = 0; i < rows_left; ++i) {
        for (int j = 0; j < cols_right; ++j) {
            // soma de produtos deve ser atômica, cria-se variável local sum
            // out[i*cols_right+j] = 0;
            double sum = 0.0;
            // evita-se paralelismo aninhado
            // #pragma omp parallel for firstprivate(i, j) schedule(guided)
            for (int k = 0; k < cols_left; ++k) {
                // incrementa-se sum localmente
